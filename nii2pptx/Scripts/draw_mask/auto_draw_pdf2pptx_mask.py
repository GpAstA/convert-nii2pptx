...
# 領域のマスキング処理（マスクを元の画像に重ねる）
def masking_and_overlay(cropped_images, original_images):
    overlay_images = []
    for cropped_image, original_image in zip(cropped_images, original_images):
        # マスクの部分だけ色をつけるため、マスクを3チャンネルに変換
        mask = cv2.cvtColor(cropped_image, cv2.COLOR_RGB2GRAY)

        # マスクのサイズが元の画像と一致しているか確認し、一致しない場合はリサイズ
        if mask.shape != original_image.shape[:2]:
            mask = cv2.resize(mask, (original_image.shape[1], original_image.shape[0]))

        # マスクのデータ型が8ビット整数であることを確認
        mask = mask.astype(np.uint8)

        # マスクの反転を作成（元の画像部分を残すため）
        mask_inv = cv2.bitwise_not(mask)

        # マスクを適用して、元の画像のマスクされた部分を抽出
        masked_image = cv2.bitwise_and(original_image, original_image, mask=mask_inv)

        # マスク部分の色を白にする
        mask_colored = cv2.cvtColor(mask, cv2.COLOR_GRAY2RGB)

        # 元の画像とマスクを重ね合わせる
        overlay_image = cv2.add(masked_image, mask_colored)
    ...
    for root, dirs, files in os.walk(input_folder):
        for file in files:
            if file.endswith(".pdf"):
                ...
                # マスキングと元画像の重ね合わせ
                overlay_images = masking_and_overlay(cropped_images, numpy_images)

                # 出力PPTXファイルパスを生成
                pptx_output_path = generate_output_pptx_path(pdf_path)

                # 画像をPPTXに保存
                save_images_to_pptx(overlay_images, pptx_output_path)
